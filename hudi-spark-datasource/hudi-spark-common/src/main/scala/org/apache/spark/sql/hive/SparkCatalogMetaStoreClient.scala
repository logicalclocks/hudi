...
import org.apache.spark.sql.catalyst.catalog.{CatalogDatabase, CatalogStorageFormat, CatalogTable, CatalogTablePartition, CatalogTableType, ExternalCatalogUtils}
...
class SparkCatalogMetaStoreClient(syncConfig: HiveSyncConfig)
  extends IMetaStoreClient with Logging {
  ...
  override def listPartitionsByFilter(dbName: String,
    ...
    // Hive filter strings generated by Hudi's PartitionFilterGenerator are plain SQL
    // predicates over partition columns, so parse them with Catalyst and prune through
    // the external catalog. Fall back to listing all partitions (a safe superset for
    // the sync diff logic) if the filter cannot be parsed/resolved.
    try {
      val catalogTable = externalCatalog.getTable(dbName, tableName)
      val partitionKeys = catalogTable.partitionColumnNames.toList
      val attrByName = catalogTable.partitionSchema.fields
        .map(f => f.name -> AttributeReference(f.name, f.dataType, f.nullable)()).toMap
      val predicate = CatalystSqlParser.parseExpression(filter).transformUp {
        case u: UnresolvedAttribute =>
          attrByName.getOrElse(u.name,
            throw new IllegalArgumentException(s"${u.name} is not a partition column"))
      }
      require(predicate.resolved, s"could not resolve partition filter: $filter")
      externalCatalog.listPartitionsByFilter(dbName, tableName, Seq(predicate),
          sparkSession.sessionState.conf.sessionLocalTimeZone)
        .map(fromCatalogPartition(_, dbName, tableName, partitionKeys)).asJava
    } catch {
      case NonFatal(e) =>
        logWarning(s"Falling back to full partition listing for $dbName.$tableName; " +
          s"could not push down partition filter [$filter]: ${e.getMessage}")
  ...
  override def add_partitions(parts: util.List[Partition], ifNotExists: Boolean, needResults: Boolean): util.List[Partition] = {
    if (parts == null || parts.isEmpty) {
      ...
      val partitionKeys = partitionKeysOf(db, tbl)
      val catalogParts = parts.asScala.map(toCatalogPartition(_, partitionKeys)).toSeq
  ...
  override def alter_partitions(dbName: String,
                                ...
                                environmentContext: EnvironmentContext): Unit = {
    val partitionKeys = partitionKeysOf(dbName, tableName)
    externalCatalog.alterPartitions(dbName, tableName,
      newParts.asScala.map(toCatalogPartition(_, partitionKeys)).toSeq)
  }
  ...
  private def partitionKeysOf(dbName: String, tableName: String): List[String] = {
    externalCatalog.getTable(dbName, tableName).partitionColumnNames.toList
  }
  ...
  private def fromCatalogPartition(part: CatalogTablePartition, db: String, table: String, partitionKeys: List[String]): Partition = {
    ...
    new Partition(values, db, table, 0, 0, sd,
      new util.HashMap[String, String](part.parameters.asJava))
  }

  private def toCatalogPartition(part: Partition, partitionKeys: List[String]): CatalogTablePartition = {
    val values = Option(part.getValues).map(_.asScala.toList).getOrElse(Nil)
    val spec = partitionKeys.zip(values).toMap
  ...
  private def parsePartitionClause(partName: String): Map[String, String] = {
    partName.split("/").flatMap { token =>
      token.split("=").toList match {
        case k :: v :: Nil =>
          // Hive partition names are path-escaped (e.g. %3A for ':'); unescape so the
          // spec matches what Spark's catalog stores.
          Some(ExternalCatalogUtils.unescapePathName(k.trim) ->
            ExternalCatalogUtils.unescapePathName(v.trim.stripPrefix("'").stripSuffix("'")))
        case _ => None
      }
    }.toMap
  }
